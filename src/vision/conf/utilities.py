...
def loadJson(filename):
    ...
    if filename == UNKNOWN_EXTENSION:
        ...
        if os.path.isfile(filename) == True:
            ...
            print("INFO: Couldn't find the Json file [ " + filename + " ], creating it now.")
...
def saveJson(json_obj, filename):
    ...
    if filename == UNKNOWN_EXTENSION:
        print("ERROR: The extension given to the file is not valid [ " + filename + " ], expected .json")
        return UNKNOWN_EXTENSION
    else:
...
def getConfigurationValues(key,filename):
    ...
    if key == "red" or key == "yellow" or key == "blue":     
        ...
        if state == SUCCESS:
            configuration = np.array([obj_json[key][0],obj_json[key][1],obj_json[key][2],obj_json[key][3],obj_json[key][4] ,obj_json[key][5]])
